...
def main() -> None:
    ...
    if args.mode == "train":
        ...
        config = EntityConfig(word_ngrams=tuple(map(int, args.word_ngrams.split(","))), char_ngrams=tuple(map(int, args.char_ngrams.split(","))), min_df=args.min_df, max_features=args.max_features, c=args.c, balanced=not args.unweighted, threshold=args.threshold, random_state=20260824, semantic_keyword_boost=args.semantic_keyword_boost, semantic_keyword_filter=args.semantic_keyword_filter, semantic_keyword_min_hits=args.semantic_keyword_min_hits, semantic_relation_boost=args.semantic_relation_boost)
