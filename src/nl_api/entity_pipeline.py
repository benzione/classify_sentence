...
class EntityPipeline:
    ...
    def __init__(self, config: EntityConfig, vectorizer: FeatureUnion, labels: list[str], *,
        ...
        self.direct_heads = direct_heads
    ...
    def train(cls, rows: list[dict[str, Any]], config: EntityConfig | None = None) -> "EntityPipeline":
        ...
        return cls(config, vectorizer, labels, direct_heads=_BinaryHeads.fit(matrix, targets, labels, config),
                   semantic_keywords=semantic_keywords, semantic_relation_rules=semantic_relation_rules, **kwargs)
    ...
    def predict_batch(self, questions: Iterable[str]) -> list[EntityPrediction]:
        ...
        return self._semantic_predictions(self.direct_heads.probabilities(matrix), normalized)
